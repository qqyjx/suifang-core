...
-- 仅供本地/开发库 (例如 h6dp_suifang_dev) 联调随访平台 1.0/1.1 用。
--
-- 安全阀: 下面这行 USE 是本文件第一条可执行语句, 硬编码指向 dev 库名。生产服务器上
-- 没有 h6dp_suifang_dev 这个库, 一旦有人不小心对着生产 MySQL 跑本文件, USE 会立刻报
-- "Unknown database" 直接失败退出, 不会往下执行任何一条 INSERT/DELETE, 比只靠注释警告更保险。

USE h6dp_suifang_dev;
...
--   5. (M5 新增) 建 platform_plan 表 + 插入 3 条随访计划(逾期循环 / 今日到期一次性 /
--      未来循环), 供 GET /api/platform/plans、GET /api/platform/tasks、
--      POST /api/platform/task/complete 联调, 见第 8 节。
--
...
-- 第 6 节 iwown_data alarm 行用 NOT EXISTS 判重, 也可安全重复执行; 第 8 节 platform_plan
-- 按 (patient_no,name,next_due) 先 DELETE 再 INSERT, 也可安全重复执行; 其余
...
-- ============ 8. (M5) platform_plan + 3 条随访计划 ============
-- 与 database/platform.sql / ensure_platform_tables() 的 DDL 保持一致 (先建表, 服务启动时
-- CREATE TABLE IF NOT EXISTS 是 no-op, 与第 2 节两张表同一惯例)。
-- dev 里的"今天" = 2026-07-16 (与本文件其余各节的 07-12..07-16 锚点一致):
--   a) DEV0001 循环计划, 已逾期 (next_due 2026-07-09, 早于今天 7 天, freq=7) —— 完成后
--      next_due 应推进为 完成当日+7, 而不是从旧到期日 07-09 起算。
--   b) DEV0002 一次性计划, 今日到期 (next_due = 2026-07-16) —— 完成后 active 应变 0。
--   c) DEV0001 循环计划, 未来到期 (next_due 2026-07-26, freq=30) —— 验证"未来 7 天"窗口
--      看不到它(仅验证 tasks?horizon_days=7 不返回), 但 plans 列表能查到。
-- 幂等做法: 按 (patient_no, name, next_due) 组合先 DELETE 再 INSERT (没有业务唯一键, 与其余
-- 直插明细表不同, 这里选择可重复执行而不是 ON DUPLICATE KEY, 因为 AUTO_INCREMENT id 允许变化,
-- 联调时依赖 id 的地方改用 patient_no+name 重新查询即可)。
...
DELETE FROM platform_plan WHERE (patient_no, name, next_due) IN (
    ('DEV0001', '术后1月电话随访', '2026-07-09'),
    ('DEV0002', '出院首次电话随访', '2026-07-16'),
    ('DEV0001', '慢病季度随访', '2026-07-26')
);

INSERT INTO platform_plan (patient_no, name, frequency_days, next_due, active, note) VALUES
    ('DEV0001', '术后1月电话随访', 7, '2026-07-09', 1, 'M5 验收用: 已逾期循环计划, 完成后 next_due 从完成当日起算'),
    ('DEV0002', '出院首次电话随访', NULL, '2026-07-16', 1, 'M5 验收用: 今日到期一次性计划, 完成后 active=0'),
    ('DEV0001', '慢病季度随访', 30, '2026-07-26', 1, 'M5 验收用: 未来到期循环计划, 7 天窗口内不应出现在待办任务');
