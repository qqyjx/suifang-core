...
Page({
  ...
  devScreenHeight: 280,
  smallDevScreenWidth: 172,
  smallDevScreenHeight: 198,
  devSmallBgWidth: 152,
  devSmallBgHeight: 178,
  devScale: 1,
  /**
   * 生命周期函数--监听页面加载
   */
  ...
  smallData(tempFilePath: any, arr: any) {
    ...
    wx.canvasGetImageData({
      ...
      success: (res) => {
        ...
        console.log("arr=>", arr)
        // self.imageTobin565Fun(arr[0].data2)
  ...
  // 去除rgba中的a
  deleteRGBA(rgbaArray: Uint8Array) {
    const rgbArray = new Uint8Array(rgbaArray.length / 4 * 3);

    for (let i = 0, j = 0; i < rgbaArray.length; i += 4, j += 3) {
      rgbArray[j] = rgbaArray[i]; // Red  
      rgbArray[j + 1] = rgbaArray[i + 1]; // Green  
      rgbArray[j + 2] = rgbaArray[i + 2]; // Blue  
    }
    return rgbArray
  },
  // 转换数据，非四字节补齐
  getTransFormData(value: any) {
    let data = value // 你需要替换为你的数据
    let count = 4 - (data.length % 4);
    // 如果不是4字节对齐，进行补齐
    if (count !== 4) {
      let fill = 0xFF; // 用十六进制的FF填充
      data = data.concat(new Uint8Array(fill).buffer);
    }
  ...
  _bmpConvert(val: any) {
    ...
    val.forEach((item: any, index: number) => {
      ...
      // 缩略图
      if (item.data1) {
        let content = self.getTransFormData(item.data1)
        let deletea = self.deleteRGBA(content)
        obj.data1 = deletea
      } else {
        // 大图
        let content = self.getTransFormData(item.data2)
        let deletea = self.deleteRGBA(content)

        obj.data2 = deletea
      }
    })
    let totalArray = [...obj.data1, ...obj.data2]
    console.log("totalArray=>",)
    console.log("obj=======>", obj)
    eventChannel.emit('onDialBgData', { data: Array.from(this.splicingData(totalArray)) });
    wx.navigateBack()

  },

  // 拼接数据
  splicingData(tempData: any) {
    const data = new Uint8Array(tempData.length * (2 / 3))
    for (let index = 0; index < tempData.length; index += 3) {
      const r = tempData[index];
      const g = tempData[index + 1];
      const b = tempData[index + 2];
      let big = (r & 0xF8) | ((g >> 5) & 0x07);
      let little = ((g << 3) & 0xe0) | ((b >> 3) & 0x1F);
      if (index == 0) {
        data[index] = big
        data[index + 1] = little
      } else {
